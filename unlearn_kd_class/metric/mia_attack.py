...
def balance_loader_by_stratified_sampling(retain_loader, test_loader, batch_size=32):
    ...
    labels = torch.tensor([label for _, label in retain_loader.dataset])
...
def entropy(p: torch.Tensor) -> torch.Tensor:
    """샘플별 엔트로피 계산"""
    return (-(p * (p.clamp(min=1e-12).log())).sum(dim=1))
...
def get_membership_attack_prob(retain_train_loader, forget_loader,
    ...
    retain_train_loader = balance_loader_by_stratified_sampling(retain_train_loader, test_loader)
    ...
    Xr = Xr + eps * np.random.randn(*Xr.shape)
    Xf = Xf + eps * np.random.randn(*Xf.shape)
