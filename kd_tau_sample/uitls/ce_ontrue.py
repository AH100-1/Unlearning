...
# ===== 실험 하이퍼 (그리드) =====
TAU_G_LIST = [0.1, 0.3, 0.5, 0.7, 1, 3, 5, 7, 10]
TAU_B_LIST = [0.1, 0.3, 0.5, 0.7, 1, 3, 5, 7, 10]
EPOCH_COUNTS = [10, 15, 20]              # 언러닝 반복 에폭
CE_TOGGLE = [True]                      # CrossEntropy on/off (retain 샘플에만 적용)
...
def main():
    ...
    # retain/forget 로더 외에, 합본 로더가 필요할 때:
            ...
            for tb in TAU_B_LIST:
                for E in EPOCH_COUNTS:
                    ...
                    for ep in range(E):
                        ...
                        print(f'  Epoch {ep+1:2d}/{E:2d}  Loss {running/len(combined_tr):.4f}')
